...
/* see inner.h */
void ZfN(poly_add_mul_fft)(fpr *restrict d,
	const fpr *restrict F, const fpr *restrict G,
	const fpr *restrict f, const fpr *restrict g, unsigned logn)
{
	// assert(logn >= 4);
    ...
    for (int i = 0; i < hn; i += 8)
    {
        ...
        FPC_MULx4(a_re, a_im, F_re, F_im, f_re, f_im);
        ...
        FPC_MLAx4(a_re, a_im, G_re, G_im, g_re, g_im);
	...
	fpr *restrict x0, fpr *restrict x1, unsigned logn)
{
    const int hawk_n = 1 << logn;
    const int hn = hawk_n >> 1;
    float64x2x4_t b00_re, b00_im, b01_re, b01_im, b10_re, b10_im, b11_re, b11_im, 
                  x0_re, x0_im, x1_re, x1_im, y0_re, y0_im, y2_re, y2_im;

    for (int i = 0; i < hn; i += 8)
    {
        vloadx4(b00_re, &b00[i]);
        vloadx4(x0_re, &x0[i]);

        vloadx4(b00_im, &b00[i + hn]);
        vloadx4(x0_im, &x0[i + hn]);

        FPC_MULx4(y0_re, y0_im, b00_re, b00_im, x0_re, x0_im);

        vloadx4(b01_re, &b01[i]);
        vloadx4(x1_re, &x1[i]);

        vloadx4(b01_im, &b01[i + hn]);
        vloadx4(x1_im, &x1[i + hn]);

        FPC_MLAx4(y0_re, y0_im, b01_re, b01_im, x1_re, x1_im);

        vstorex4(&x0[i], y0_re);
        vstorex4(&x0[i + hn], y0_im);

        vloadx4(b10_re, &b10[i]);
        vloadx4(b10_im, &b10[i + hn]);

        FPC_MULx4(y2_re, y2_im, b10_re, b10_im, x0_re, x0_im);

        vloadx4(b11_re, &b11[i]);
        vloadx4(b11_im, &b11[i + hn]);

        FPC_MLAx4(y2_re, y2_im, b11_re, b11_im, x1_re, x1_im);

        vstorex4(&x1[i], y2_re);
        vstorex4(&x1[i + hn], y2_im);
    }
}
